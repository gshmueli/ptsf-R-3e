...
# Fit model with seasonality & quadratic trend
...
train.res.arima <- train.lm.trend.season.resid |> 
  model(ARIMA(.resid ~ 1 + pdq(1,0,0)))  # '1' is used to include a constant in ARIMA model

train.res.arima.fitted <- fitted.values(train.res.arima)

train.lm.trend.season.resid  |>
  autoplot() + 
  ...
  geom_segment(aes(x = yearmonth("2001-May"), y = 200, xend = yearmonth("2004-May"), yend = 200),
  ...
  geom_segment(aes(x = yearmonth("1991-Jan"), y = 200, xend = yearmonth("2001-Mar"), yend = 200),
               arrow = arrow(length = unit(0.25, "cm"), ends = "both"), color="grey55")+  
...
### AR(1) output and one-step-ahead forecast
report(train.res.arima)
train.res.arima |> forecast(h = 1)

################ Figure 7.5
train.res.arima |> residuals() |> 
