...
rain.df <- read.csv("Data/MelbourneRainfall.csv") # read the data. The data frame contain two columns: Date and RainfallAmount_millimetres
rain.df$Date <- as.Date(rain.df$Date, format="%d/%m/%Y") # change the data format in the Date column.
rain.df$Rainy <- ifelse(rain.df$Rainfall > 0, 1, 0) # create new binary column Rainy (1=rainy day).
...
rainy.lr.pred <- predict(rainy.lr, xvalid, type = "response") # type="response" gives probabilities
