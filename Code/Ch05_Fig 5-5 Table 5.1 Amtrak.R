#################
# Code to create Figure 5-5 and Table 5.1
...
fc.ets |> 
  ...
  geom_segment(aes(x = yearmonth("2001-May"), y = 250, 
                   xend = yearmonth("2004-Mar"), yend = 250),
  ...
  geom_segment(aes(x = yearmonth("1991-Jan"), y = 250, 
                   xend = yearmonth("2001-Mar"), yend = 250),
               arrow = arrow(length = unit(0.25, "cm"), ends = "both"), color="grey55")+  # error Validation
...
# train.ridership$dif12_1 - train.ts
...
############################################
############################################
fit.ets.opt <- train.ridership |> model(ets =  ETS(dif12_1 ~ error("A") + trend("N") + season("N")) )
report(fit.ets.opt)

fc.ets.opt <- fit.ets.opt |> forecast(h=dim(valid.ridership)[1])

accuracy(fit.ets) # train.ridership
accuracy(fc.ets , valid.ridership)

accuracy(fit.ets.opt ) # train.ridership
accuracy(fc.ets.opt , valid.ridership)
